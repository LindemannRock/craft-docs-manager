...
/**
 * PHPUnit bootstrap for the docs-manager plugin.
 *
...
if (!function_exists('craft_modify_app_config')) {
    ...
    {
        if ($appType !== 'console') {
            throw new \RuntimeException('Docs Manager tests require Craft\'s console application.');
        }

        $queueConfig = $config['components']['queue'] ?? [];
        if (!is_array($queueConfig)) {
            throw new \RuntimeException('Docs Manager tests require an array-configured Craft queue.');
        }
