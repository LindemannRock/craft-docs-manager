...
final class IsolatedQueue extends Queue
{
    ...
    public function init(): void
    {
        ...
        if ($this->db->getDriverName() !== 'mysql') {
            throw new \RuntimeException('The disposable Docs Manager suite currently requires MySQL.');
        }

        $this->rawShadowTable = $this->db->getSchema()->getRawTableName($this->tableName);
        $stagingTable = $this->rawShadowTable . '_dm_bootstrap_' . bin2hex(random_bytes(8));
        ...
        register_shutdown_function(function(): void {
            ...
            try {
                ...
                fwrite(STDERR, 'Docs Manager queue-shadow cleanup failed: ' . $exception->getMessage() . PHP_EOL);
            }
        });
    }
